...
| **2.2** | PROPOSE_HOP_PLAN | ❌ NOT IMPLEMENTED | Agent completes hop design | `hop_designer_node` → stateTransitionApi.proposeHopPlan() | No changes | No changes | Updates `status=HOP_PLAN_PROPOSED`, `updated_at=now()` | No changes | **Creates hop-scoped input assets (copied from mission inputs), creates hop-scoped output assets (based on output_asset_spec)** | Agent completes design and proposes to user. Hop moves to proposed state. Assets created based on completed hop specification |
