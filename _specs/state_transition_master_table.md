...
| **2.1** | START_HOP_PLAN | ❌ NOT IMPLEMENTED | User requests hop planning via chat | User message → `hop_designer_node` | No changes | Sets `current_hop_id=hop_id`, `updated_at=now()` | Creates with `status=HOP_PLAN_STARTED` | No changes | **Creates hop-scoped input assets (copied from mission inputs), creates hop-scoped output assets (based on output_asset_spec)** | User initiates hop planning. Hop created in started state. Agent begins planning |
| **2.2** | PROPOSE_HOP_PLAN | ❌ NOT IMPLEMENTED | Agent completes hop design | `hop_designer_node` → stateTransitionApi.proposeHopPlan() | No changes | No changes | Updates `status=HOP_PLAN_PROPOSED`, `updated_at=now()` | No changes | No changes | Agent completes design and proposes to user. Hop moves to proposed state |
