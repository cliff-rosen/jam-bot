...
const CollabArea: React.FC<CollabAreaProps> = ({ type, content }) => {
    ...
    const renderApprovalContent = () => {
        ...
        switch (type) {
            case 'mission-proposal':
                ...
                // Check if mission is already in progress (replace the executing hop check)
                const isAlreadyAccepted = state?.mission?.status === MissionStatus.IN_PROGRESS;
                return (
                    <MissionProposal
                        mission={mission}
            ...
            case 'hop-proposal':
                ...
                // Check if this hop has already been accepted (is beyond proposal phase)
                const existingHop = state?.mission?.hops.find((existingHop: any) =>
                    existingHop.id === hop.id
                );
                const isHopAlreadyAccepted = existingHop &&
                    existingHop.status !== HopStatus.HOP_PLAN_PROPOSED &&
                    existingHop.status !== HopStatus.HOP_IMPL_PROPOSED;
                const needsAcceptance = hop.status === HopStatus.HOP_PLAN_PROPOSED && !isHopAlreadyAccepted;
                return (
                    <HopProposal
                        hop={hop}
    ...
    const renderCurrentHop = () => {
        ...
        if (currentHop.status === HopStatus.HOP_IMPL_READY) {
            actionButtons.push(
                <button
                    key="start"
